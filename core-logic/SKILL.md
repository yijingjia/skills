...
## 输出格式强约束 (Output Contract)
为避免用户二次追问“你用的是书里的什么理论”，**每一次首次分析输出**都必须在末尾追加独立小节 **「分析所依据的书内章节与理论」**，并**按下列结构**填写（只列本次分析实际用到的章节与要点）。
...
### 2. 按章节逐条列出（每条包含以下四部分）
- **章节名**  
  章节名以本技能库下的 `references/index.json` 中 `chapters` 的 `title` 为准。
- **用到的书中要点**：（ bullet 列表，只列本技能从该章用到的要点，供用户对照书阅读。）
- **对应我分析里的**：（一两句话说明：你在上文分析中哪一段、哪一结论用到了这些要点。）

- **只列本次实际使用到的章节**，不要堆砌未用到的依据。
- 对用户只输出可读的章节名、要点与对应关系；如需溯源，可在「对应技能」中列出技能文件名。
